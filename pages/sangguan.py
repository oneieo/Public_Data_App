...
# Y 변수: 반려동물 등록수 / (펫파크수 + 1)  [수요 높고 공급 부족한 곳]
...
plt.tight_layout()
plt.savefig('상관분석_히트맵.png', dpi=150)
plt.show()
...
print('-'*35)
for v, vif in zip(X_vars, vifs):
    judge = '✅ 정상' if vif < 5 else ('⚠️ 주의' if vif < 10 else '❌ 제거 필요')
    print(f'{v:<12} {vif:>8}  {judge}')
